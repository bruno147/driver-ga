...
float InsideTrack::targetSpeed = 300;
float InsideTrack::distance = 0;

InsideTrack::InsideTrack() : currentGear(START_GEAR), safeDist(100) {}
...
void InsideTrack::setTargetSpeed(CarState &cs) {
    ...
    int disFromStart = cs.getDistFromStart();
    if(memory.empty())    return;
    else {
        for (int i = 0; i < (int)memory.size(); ++i) {
            if(memory.at(i).landmark > disFromStart) {
                if((memory.at(i).landmark - disFromStart) < safeDist) {
                    if(this->targetSpeed > memory.at(i).targetSpeed)
                        this->targetSpeed = memory.at(i).targetSpeed;
                }
            }
        }
    }
